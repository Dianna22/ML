    ...
    def train(self, input_set, target_set, iterations, learningRate, batches, f, regularization_param, xt,yt):
        ...
        while iterations > 0:
            ...
            for input_layer, target in zip(input_set, target_set):
                ...
                if count % batches == 0:
                    batch += 1
                    self.weights_hidden_output = np.subtract(self.weights_hidden_output, np.multiply(delta["hidden_output"], learningRate))
                    self.last_layer_bias = np.subtract(self.last_layer_bias, np.multiply(delta_bias["hidden_output"], learningRate))
                    self.weights_input_hidden = np.subtract(self.weights_input_hidden, np.multiply(delta["input_hidden"], learningRate))
                    self.hidden_bias = np.subtract(self.hidden_bias, np.multiply(delta_bias["input_hidden"], learningRate))
    def predict(self, x):
